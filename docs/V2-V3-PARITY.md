...
| Compile-time CUDA smoke (no v2 equivalent) | Write trivial `smoke.cu` + run `nvcc -o smoke smoke.cu` after install to prove the toolkit headers/libs actually work. | implemented | `internal/phase.Cuda.runSmokeIfRequested` + `CudaSmokeProgram` const | Compile failure is fatal in required mode; running the binary is best-effort and never fatal (cloud images may have no CUDA device pre-reboot). State.Details records both compile + run outcomes. |
