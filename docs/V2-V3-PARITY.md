...
| CUDA policy (`install_cuda_toolkit_if_requested`) | `mode = none / optional / required` (hdr-4k120 uses `none`). | implemented | `internal/cuda.Plan` + `internal/phase.Cuda` | Apt candidate discovery added in M3.5. |
| CUDA runfile retry-suppression (`install_cuda_toolkit_from_runfile`) | Refuses to redownload the same SHA after a failed `--check`. | partial | `internal/cuda.RetryDecider` | Decider exists; runfile install path is not yet wired. Acceptable because hdr-4k120 uses `mode=none`. |
