...
| CUDA policy (`install_cuda_toolkit_if_requested`) | `mode = none/optional/required`, `method = auto/apt/runfile/none`, `selection_policy = latest-compatible/exact-major/min-major/any`, `expected_major`, `min_major`, `allow_ubuntu_archive_fallback`, `compile_smoke_test`. | implemented | `internal/cuda.{Plan,ParseMethod,ParseSelectionPolicy,Selection,CandidateLadder}` + `internal/phase.Cuda` | Four profiles ship: `hdr-4k120` (no CUDA), `hdr-4k120-cuda` (strict 13, 25.10, currently unsatisfiable), `hdr-4k120-cuda-ubuntu2404` (strict 13, 24.04 LTS, known-working), `hdr-4k120-cuda-compatible` (latest-compatible, min 12, archive fallback). |
| Compile-time CUDA smoke (no v2 equivalent) | Write trivial `smoke.cu`, run `nvcc -o smoke smoke.cu`, then execute the smoke binary under hard timeouts. | implemented | `internal/phase.Cuda.runSmokeIfRequested` + `CudaSmokeProgram` const | Required mode fails cleanly on compile/run timeout or failure. Optional/auto profiles mark CUDA degraded/skipped and continue because KMS/NVENC/Sunshine validation is independent of the toolkit smoke binary. State.Details records stage timings and stdout/stderr tails. |
...
| Sunshine fork clone / build / pin (`install_sunshine_from_fork_if_requested`) | Pins the CloudDeploy Sunshine fork, builds, dual-installs, setcap. | real | `internal/phase/milestone5.go` | v3 profiles pin `8f02b1ce` for HDR, Linux `audio_sink`, and GPL notice fixes. |
