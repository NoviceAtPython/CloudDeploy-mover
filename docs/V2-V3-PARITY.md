...
| CUDA policy (`install_cuda_toolkit_if_requested`) | `mode = none / optional / required`, `method = auto / apt / runfile / none`. | implemented | `internal/cuda.Plan` + `internal/cuda.ParseMethod` + `internal/phase.Cuda` | `hdr-4k120` keeps `mode=none` (KMS+NVENC HDR does not need CUDA); `hdr-4k120-cuda-ubuntu2404` is the known-working `mode=required` test target (`hdr-4k120-cuda` itself targets 25.10 and is currently unsatisfiable — NVIDIA has no ubuntu2510 CUDA apt repo and the 13.0.2 runfile is corrupt). |
