...
| `phase cuda` | **Real (policy-driven, source-aware layout).** Honors `cuda.method` (auto/apt/runfile/none) + `cuda.selection_policy` (latest-compatible / exact-major / min-major / any) + `cuda.expected_major` + `cuda.min_major` + `cuda.allow_ubuntu_archive_fallback` + `cuda.compile_smoke_test`. Apt path bootstraps `cuda-keyring_1.1-1` on hosts with an NVIDIA CUDA apt repo; falls through to Ubuntu archive when allowed AND no NVIDIA repo. Post-install verification is **layout-aware**: NVIDIA-repo / runfile installs must produce `/usr/local/cuda/{bin/nvcc,include,lib64}`; Ubuntu archive installs are validated against `/usr/bin/nvcc` + (`/usr/include/cuda_runtime.h` OR `/usr/lib/cuda/include`) + (`/usr/lib/x86_64-linux-gnu/libcudart.so*` OR `/usr/lib/cuda/lib64`). `cuda.compile_smoke_test=true` compiles a trivial `.cu` with the freshly-installed nvcc (fatal in required mode); running the binary is best-effort. State.Details records `source`, `selected_package`, `selected_major`, `driver_preferred_major`, `required_major`, `min_major`, `layout_kind` ("nvidia-canonical" / "ubuntu-archive"), `layout_nvcc`, `layout_headers`, `layout_libs`, `archive_fallback`, `compile_smoke_test*`, `runtime_smoke_test*`. |
| `phase ubuntu-upgrade` | **Real, resume-friendly.** Records a `stage` marker in state.Details as it progresses: `started → third-party-sources-disabled → sources-rewritten → dist-upgrade-started → dist-upgrade-complete → reboot-required`. The anti-reboot-loop guard fires ONLY when stage indicates the previous run completed dist-upgrade AND `current==pre_version`; earlier stages re-enter idempotently. `rewriteAptCodename` accepts source files that already mention the target codename. Before mutating sources the phase stops `apt-daily.timer`, `apt-daily-upgrade.timer`, `unattended-upgrades.service`, and waits up to 3 minutes for any in-flight dpkg/apt holders to release. |
...
| `hdr-4k120-cuda-compatible` | 25.10 | broad: `latest-compatible`, `min_major=12`, archive fallback ok, compile smoke on | Broad-compat profile. On Ubuntu 25.10 (no NVIDIA CUDA repo today) the apt path falls through to Ubuntu archive's `nvidia-cuda-toolkit` (CUDA 12.4) without bootstrapping cuda-keyring. State records `repo_distro: "ubuntu-archive"` + `archive_fallback: true`. |
