...
Five test profiles cover the supported paths:
...
| `hdr-4k120-cuda` | 25.10 | strict: `exact-major=13`, cross-distro ladder `[auto-host, ubuntu2404]`, no archive fallback | Strict CUDA-13. On 25.10 the apt path probes `ubuntu2510` (missing today), then `ubuntu2404` (reachable), bootstraps the keyring from ubuntu2404, and installs `cuda-toolkit-13-N`. |
| `hdr-4k120-cuda-ubuntu2404` | 24.04 | strict: `exact-major=13`, native-only (auto-host = ubuntu2404), no archive fallback | LTS strict variant. |
| `hdr-4k120-cuda-compatible` | 25.10 | broad: `latest-compatible`, `prefer_major=13`, `min_major=12`, cross-distro ladder, archive fallback ok | Broad-compat. Cross-distro CUDA 13 from ubuntu2404 beats Ubuntu archive's CUDA 12.4; archive is a LAST resort. |
| `hdr-4k120-cuda-native` (this commit) | 25.10 | strict: `exact-major=13`, **no cross-distro**, no archive fallback | Diagnostic profile: only probes `auto-host`. A failure answers "does NVIDIA officially support this Ubuntu version yet?". |
