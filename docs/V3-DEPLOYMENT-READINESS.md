...
| `phase cuda` | **Real (policy-driven).** Honors `cuda.method` (auto/apt/runfile/none) AND `cuda.selection_policy` (latest-compatible / exact-major / min-major / any) + `cuda.expected_major` + `cuda.min_major` + `cuda.allow_ubuntu_archive_fallback`. Apt path: if the host's Ubuntu has an NVIDIA CUDA apt repo, bootstrap `cuda-keyring_1.1-1` and install toolkit-only (refuses `cuda-drivers*`); when no NVIDIA CUDA repo exists for this Ubuntu AND `allow_ubuntu_archive_fallback=true`, skip the keyring bootstrap entirely and resolve a candidate from the host's existing apt sources (Ubuntu archive). State records `archive_fallback=true` + `repo_distro="ubuntu-archive"` in that case. The candidate ladder is version-range aware (`cuda-toolkit-14-N → 13-N → 12-N → cuda-toolkit → nvidia-cuda-toolkit`) and policy-filtered (exact-major emits only that major and never the archive unless the archive's major matches). Runfile path: `/var/tmp/clouddeploy-cuda`, curl-retries, size + sha256 logging, `--check` before install, `RetryDecider` refuses same-SHA retry, `sh runfile --silent --toolkit --override --tmpdir=...`. Post-install verification: nvcc parse + `/usr/local/cuda/{bin/nvcc,include,lib64}` + policy-driven `Selection.SatisfiesNvcc(rel)`. When `cuda.compile_smoke_test=true`, write `/var/tmp/clouddeploy-cuda-smoke/smoke.cu`, compile with the freshly-installed nvcc (compile failure is fatal in required mode), and best-effort run (runtime failure logged, never fatal). State.Details records source / package / repo_distro / archive_fallback / runfile_url / sha256 / nvcc_version / cuda_major / expected_major / selection_policy / verified_layout / compile_smoke_test* / runtime_smoke_test* / profile_snippet. |
