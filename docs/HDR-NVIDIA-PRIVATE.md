...
| Single `require_patched_kwin_if_hdr` runs *after* `build_install_patched_kwin`. Checks both: (a) `ENABLE_HDR=1` implies `KWIN_CLOUDDEPLOY_NVIDIA_PRIVATE_HDR=1` (catches the misconfiguration case where the build was skipped because the operator turned off the patched path) and (b) `${PATCHED_KWIN_MARKER}` exists (catches build-pipeline failures). No early guard, so a fresh `ENABLE_HDR=1` VM is always allowed to build on its first pass. | **Done** |
| Post-deploy `validate_hdr_final_state` walks `drm_info -j` for an ACTIVE=1 CRTC and its active primary plane (CRTC_ID matching, FB_ID != 0) via `scripts/validate-hdr-drm-state.py` — never a global grep. Requires `NV_CRTC_REGAMMA_TF=PQ` on the active CRTC + `NV_INPUT_COLORSPACE=BT.2100 PQ` + `NV_PLANE_DEGAMMA_TF=PQ` on the active primary plane; fails loudly on miss. `NV_HDR_STATIC_METADATA=blob 0` is the expected good state and logged as OK. | **Done** |
| `clouddeploy_repo_asset_path` / `require_clouddeploy_repo_asset` auto-locate the patch and Python helper. Search order: script dir, `${CLOUDDEPLOY_REPO_DIR}/`, `/usr/local/share/clouddeploy/`, then fetch from `origin/${CLOUDDEPLOY_REPO_BRANCH}` via gh CLI → GH_TOKEN-authed curl → public raw URL. Cached at `/var/lib/clouddeploy/repo-cache/`. Works whether the script is run from a full clone or a raw curl. | **Done** |
